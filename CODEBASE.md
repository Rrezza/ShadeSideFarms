...
### `land.js`
Land management hub with 7 sub-tabs: plots, fertilizer inventory, amendment application log, soil tests, crop tracking, watering, and water tests. Harvest flow is two-step: farmhands log the event (quantity only), managers allocate via modal to DB-driven destinations. The crop tracking tab has three named panes: "New Plantings" (form + filter), "Pending Allocations" (unallocated harvests), and "Crop Tracking" (group cards). Key globals: `landPlots`, `landFerts`, `landCrops`, `landHarvests`, `landAllocations`, `landDestinations`, `landPlotObservations`, `landWatering`, `landTests`, `landActiveTab`, `obsExpandedCrops`. Key functions: `loadLandPage()`, `renderLandCrops()`, `renderUnallocatedPanel()`, `submitHarvestEvent()`, `openHarvestAllocModal()`, `submitHarvestAlloc()`, `submitPlotObservation()`, `refreshPlotObs()`, `patchPlotObs()`, `deletePlotObs()`. Reads: plots, fertilizers, crops, crop_groups, crop_harvest_events, harvest_allocations, harvest_destinations, plot_observations, soil/water tests tables.

### `overview.js`
Dashboard landing page. Read-only cards per active plot and per active animal group. Plot cards show current crops, recent activity (incl. last plot observation), and cumulative metrics. Both sections (Plots and Animal Groups) are collapsible via `toggleOvSection()`. Shows commercial and learning animal groups. Key globals: `ovPlots`, `ovCrops`, `ovHarvests`, `ovFertApps`, `ovWatering`, `ovSoilTests`, `ovPlotObservations`, `ovPlotsCollapsed`, `ovAnimalsCollapsed`. Key functions: `loadOverviewPage()`, `safeFetchOv()`, `toggleOvSection()`. Reads: `field_plots`, `plot_crops`, `crop_groups`, `crop_harvest_events`, `crop_observations`, `fertilizer_applications`, `watering_events`, `soil_tests`, `fertilizers`, `crops`, `plot_observations`.
